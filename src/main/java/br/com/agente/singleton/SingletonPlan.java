...
@Plan
public class SingletonPlan extends ModificadorPlan implements java.io.Serializable {
	...
	public SingletonPlan() {
		System.out.println("\nPlan instanciado com Pattern Singleton");
	}

	@PlanBody
	/**
	 * modificação perante a classe para ter Singleton, é bem simplificada e sem muitas etapas
	 * */
	public boolean modificador() {
		try {
			...
			getCrenca().getExtrator().getClasseOrigem().addField(getCrenca().getType(), "singleton",
					(Modifier[]) Arrays.asList(Modifier.PRIVATE, Modifier.STATIC).toArray());

			/** procura se tem construtor */
			List<?> membros = getCrenca().getExtrator().getClasseOrigem().getMembers().stream()
			...
			block.addStatement("if(singleton == null){singleton = new " + getCrenca().getNomeClasse() + "();}")
